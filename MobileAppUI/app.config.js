...
const getEnvValue = (name, fallback = '') => {
  if (Object.prototype.hasOwnProperty.call(envValues, name)) {
...
if (fs.existsSync(envPath)) {
  ...
  envValues = dotenv.parse(fs.readFileSync(path.resolve(__dirname, '.env')));
} else {
  console.warn(`⚠️ app.config: no env file found. Please create ${envFile} or .env`);
}

module.exports = ({ config }) => {
  const bundleIdentifier = 'com.ceren.ceren';
  const androidPackage = 'com.ceren.ceren';
  ...
  const publicApiUrl = getEnvValue('EXPO_PUBLIC_API_URL', config.extra?.EXPO_PUBLIC_API_URL || '');
  ...
  if (!basePlugins.includes('@react-native-community/datetimepicker')) {
  ...
  try {
    ...
    const hasMapsPlugin = basePlugins.some((plugin) => Array.isArray(plugin) && plugin[0] === 'react-native-maps');
  ...
  return {
    ...
    extra: {
      ...
      GOOGLE_MAPS_API_KEY: iosKey || androidKey || config.extra?.GOOGLE_MAPS_API_KEY || '',
